...
foreach ($dataarr as $key=>$value) {
	...
	foreach ($value as $item) {
		if($key != '0') {
			$parent = get_term_by('name', $categories[$key], 'decisions_branch')->term_id;
		}

		if($categories[$key] == $item[1]) continue;

		if($parent) {
			$term = wp_insert_term($item[1], 'decisions_branch', array(
				'parent' => $parent
			));
		}else {
			$term = wp_insert_term($item[1], 'decisions_branch');
		}

		try {
			if(isset($term['term_id'])) {
				update_term_meta($term['term_id'], 'real_id', $item[0]);
			}
		}catch (Error $e) {
